...
<div>
<h3>🏠 ${houseDeal.get(0).apartmentName}</h3>
	<!-- 찜 버튼 기능은 로그인 정보가 있는 경우만 활성화 -->
	<c:if test="${not empty userinfo}">
	  <button type="button" id="likeBtn" class="btn" onclick="doLike()">찜하기</button>
	<script>
	  let iLikeThis = 0;
	  let aptCode = ${requestScope.houseDeal.get(0).aptCode};
	  let likeBtn = document.getElementById("likeBtn");
	
	  fetch("/house/like/check/" + aptCode, {
	    method: "GET",
	  })
	  ...
	  .then(data => {
	    if (data === 1) {
	      iLikeThis = 1;
	      likeBtn.innerHTML = "찜취소";
	      likeBtn.classList.add("btn-danger");
	      likeBtn.classList.remove("btn-primary");
	    } else {
	      iLikeThis = 0;
	      likeBtn.innerHTML = "찜하기";
	  ...
	  function doLike() {
	    toggleLike(iLikeThis === 1);
	  }
	
	  function toggleLike(isLike) {
		  let url = isLike
		    ? "/house/like/delete/" + aptCode
		    : "/house/like/add/" + aptCode;
		  ...
		  fetch(url, { method: 'POST' })
		    .then(response => {
		      ...
		      iLikeThis = isLike ? 0 : 1;

		      // Toggle class and innerHTML based on the new like status
		      if (iLikeThis === 1) {
		        likeBtn.classList.remove("btn-primary");
		        likeBtn.classList.add("btn-danger");
		        likeBtn.innerHTML = "찜취소";
